...
RegisterNetEvent('god_addonaccount:setMoney')
AddEventHandler('god_addonaccount:setMoney', function(society, money)
...
function EnableSocietyMoneyHUDElement()
	...
	TriggerEvent('god_society:toggleSocietyHud', true)
end
...
function DisableSocietyMoneyHUDElement()
	...
	TriggerEvent('god_society:toggleSocietyHud', false)
end
...
function UpdateSocietyMoneyHUDElement(money)
	...
	TriggerEvent('god_society:setSocietyMoney', money)
end
...
function OpenBossMenu(society, close, options, jobnum)
	options = options or {}
	local elements = {}

	--ESX.TriggerServerCallback('god_society:isBoss', function(isBoss)
		--if isBoss then
			...
			if options.washOrga then
				table.insert(elements, {label = _U('wash_money_orga'), value = 'wash_money_orga'})
			end
			...
			ESX.UI.Menu.Open('native', GetCurrentResourceName(), 'boss_actions_' .. society, {
				...
				if data.current.value == 'withdraw_society_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
						...
						if amount == nil then
							...
							menu2.close()
							TriggerServerEvent('god_society:withdrawMoney', society, amount)
							--TriggerServerEvent('retraitArgentsociete', society, amount)
						end
				...
				elseif data.current.value == 'deposit_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'deposit_money_amount_' .. society, {
						...
						if amount == nil then
							...
							menu2.close()
							TriggerServerEvent('god_society:depositMoney', society, amount)
							--TriggerServerEvent('depotArgentsociete', society, amount)
						end
					end, function(data2, menu2)
						menu2.close()
					end)
				elseif data.current.value == 'wash_money' then
					ESX.TriggerServerCallback('god_society:getBlackMoney', function(blackmoney)
						 if blackmoney == true then
						 		ESX.ShowNotification(_U('invalid_wash'))
						 else
								ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_' .. society, {
									...
									if amount == nil or amount <= 0 or amount > 12000 then
										ESX.ShowNotification(_U('invalid_amount_wash'))
									else
										menu2.close()
										TriggerServerEvent('god_society:washMoney', society, amount)
										menue2.close()
									end
								end, function(data2, menu2)
									menu2.close()
				...
				elseif data.current.value == 'wash_money_orga' then
					ESX.TriggerServerCallback('god_society:getBlackMoney', function(blackmoney)
						 if blackmoney == true then
						 		ESX.ShowNotification(_U('invalid_wash'))
						 else
								ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'wash_money_amount_' .. society, {
									...
									if amount == nil or amount <= 0 or amount > 500000 then
										ESX.ShowNotification(_U('invalid_amount_wash_orga'))
									else
										menu2.close()
										TriggerServerEvent('god_society:washMoney', society, amount)
										menue2.close()
...
function OpenRecruitMenu(society)
	ESX.TriggerServerCallback('god_society:getOnlinePlayers', function(players)
		local elements = {}
...
function OpenRecruitMenu2(society)
	ESX.TriggerServerCallback('god_society:getOnlinePlayers2', function(players)
		local elements = {}
...
function OpenPromoteMenu(society, employee)
	ESX.TriggerServerCallback('god_society:getJob', function(job)
		local elements = {}
...
function OpenPromoteMenu2(society, employee)
	ESX.TriggerServerCallback('god_society:getJob2', function(job2)
		local elements = {}
		...
		ESX.UI.Menu.Open('native', GetCurrentResourceName(), 'manage_grades_' .. society, {
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
				...
				elseif amount > Config.MaxSalary then
					...
					ESX.TriggerServerCallback('god_society:setJobSalary2', function()
						OpenManageGradesMenu2(society)
					end, society, data.current.value, amount)
				end
			end, function(data2, menu2)
...
AddEventHandler('god_society:openBossMenu', function(society, close, options, jobnum)
	OpenBossMenu(society, close, options, jobnum)
end)
