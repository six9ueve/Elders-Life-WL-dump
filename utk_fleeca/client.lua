            ...
            while true do
                ...
                if dst < 40 then
                    ...
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "[~r~E~w~] Prendre le Cash", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "[~r~E~w~] Prendre le Cash", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "[~r~E~w~] Prendre le Cash", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        ESX.ShowAdvancedNotification("INFORMATION", "~b~BRAQUAGE EN COURS", "Braquage Fleeca\n~r~INTERVENTION DEMANDEE, INDIVDU ARME", "CHAR_CALL911", 4)
...
AddEventHandler("utk_fh:freezeDoors", function()
    ...
    ESX.TriggerServerCallback('gksphone:namenumber', function(Races)
        ...
        TriggerServerEvent('gksphone:gkcs:jbmessage', name, Races[1].phone_number, 'Braquage Banque', 'img url', GPS, '["police"]', false)
        TriggerServerEvent('gksphone:gkcs:jbmessage', name, Races[1].phone_number, 'Braquage Banque', 'img url', GPS, '["sheriff"]', false)
        end)
    end
    ...
    Citizen.CreateThread(function()
        while true do
            interval = 750
            if PlayerData.job.name == "police" or PlayerData.job.name == "sheriff" and not dooruse then
                local pcoords = GetEntityCoords(PlayerPedId())
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            interval = 1
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Ouvrir la porte", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "[~r~E~w~] Fermer la porte", 0.40)
                            end
...
AddEventHandler("utk_fh:reset", function(name, data)
    ...
    Check[name] = false
    ESX.ShowNotification("La porte du coffre se referme dans 30 secondes!")
    Citizen.Wait(30000)
    ESX.ShowNotification("La porte du coffre se referme!")
...
AddEventHandler("utk_fh:startheist", function(data, name)
    ...
    exports['progressBars']:startUI(2000, "Utilise la carte Traffiqué...")
    Citizen.Wait(1500)
    ...
    disableinput = false
    Citizen.Wait(1000)
    Process(UTK.hacktime, "Hack en cours")
    ESX.ShowNotification("Hack terminé!")
...
Citizen.CreateThread(function()
    while true do
        ...
        if not done then
            ...
            if dst < 4 then
                interval = 1
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "[~r~E~w~] Utiliser une carte d'identité sécurisée", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    ESX.TriggerServerCallback("utk_fh:checkSecond", function(result)
                        ...
                        elseif not result then
                            ESX.ShowNotification("Tu n'as pas de pass de securité.")
            ...
            for k, v in pairs(UTK.Banks) do
                if not v.onaction then
                    ...
                    if dst <= 5 and not Check[k] then
                        interval = 1
                        DrawText3D(v.doors.startloc.x, v.doors.startloc.y, v.doors.startloc.z, "[~r~E~w~] Commencé le braquage de Banque", 0.40)
