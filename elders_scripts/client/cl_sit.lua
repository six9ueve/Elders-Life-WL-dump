...
local function sit(object, modelName, data, pos)
	...
	ESX.TriggerServerCallback('fl_sit:getPlace', function(occupied)
		if occupied and ConfigSit.Sitable[modelName].bench then
...
Citizen.CreateThread(function()
	ConfigSit.Interactables = {}
	for PropName,_ in pairs(ConfigSit.Sitable) do
		if type(PropName) == 'number' then
			ConfigSit.Interactables[PropName] = true
		else
			ConfigSit.Interactables[GetHashKey(PropName)] = true
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		local object, distance = ESX.Game.GetClosestObject(GetEntityCoords(playerPed), ConfigSit.Interactables)
		...
		if not somethingIsAlreadyDisplayed then
			...
			if distance < 1.5 then
				...
				for k,v in pairs(ConfigSit.Sitable) do
...
AddEventHandler('fl_sit:updateSit', function()
	...
	if distance < 1.5 then
		local offset = anims[currentAnim].offset + ConfigSit.Sitable[currentModelName].offset
