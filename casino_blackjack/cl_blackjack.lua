            ...
            if closestChair ~= nil and closestChairDist < 2 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        ...
                        drawNativeNotification("Ce siège est occupé.")
                    end
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
            ...
            AddTextComponentString("Votre main -> [~b~"..tostring(currentHand).."~s~], La main de la Banque -> [~b~"..tostring(dealersHand).."~s~]")
    ...
    while true do 
        if waitingForBetState then
            ...
            if IsControlJustPressed(0, 201) then --Submit bet [Enter]
                ...
                if tonumber(currentBetAmount) >= 0 and tonumber(currentBetAmount) <= 1000 then
                    ...
                    notify("~r~Montant Invalide (entre 0 et 1000).")
                end
            end
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "Le but du Blackjack est de battre la main du croupier sans dépasser 21.\n Ce jeu utilise quatre jeux de 52 cartes, qui sont mélangés au début de chaque main..\nLe Dealer s'arretera a 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    ClearHelp(true)
    drawNativeNotification("Placer votre mise")
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Pas de pari, le round commence...")
        end
    end)
end)
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Attend que votre tour commence...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackWin",function(tableId)
    if closeToCasino then
        ...
        happyIWon()
        canExitBlackjack = true
        drawNativeNotification("~g~Vous avez gagné!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
