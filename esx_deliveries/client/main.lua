...
function LoadWorkPlayerSkin(deliveryType)
	...
	TriggerEvent('skinchanger:getSkin', function(skin)
			if skin.sex == 0 then				
					TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['livreur'].male)				
			else		
					TriggerEvent('skinchanger:loadClothes', skin, Config.Uniforms['livreur'].female)
			end
	end)
end
	...
	if CurrentStatus ~= Status.DELIVERY_INACTIVE then
		...
		if CurrentStatus == Status.PLAYER_REMOVED_GOODS_FROM_VEHICLE then
			...
			if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, DeliveryLocation.Item2.x, DeliveryLocation.Item2.y, DeliveryLocation.Item2.z, true) < 1.5 then
				...
				TriggerServerEvent("god_deliveries:finishDelivery:server", CurrentType)
				PlaySound(-1, "Menu_Accept", "Phone_SoundSet_Default", false, 0, true)
...
function StartDelivery(deliveryType)
	TriggerServerEvent("god_deliveries:removeSafeMoney:server", deliveryType)
end
...
function SpawnDeliveryVehicle(deliveryType)
	...
	if deliveryType == 'scooter' then
		...
		SetVehicleNumberPlateText (CurrentVehicle, 'ANPE')
		exports["elders_life"]:SetFuel(CurrentVehicle, 100)	
...
function FinishDelivery(deliveryType, safeReturn)
	...
	TriggerServerEvent("god_deliveries:returnSafe:server", deliveryType, safeReturn)
...
Citizen.CreateThread(function()
	...
	TriggerServerEvent("god_deliveries:getPlayerJob:server")
end)
...
RegisterNetEvent('god_deliveries:setPlayerJob:client')
RegisterNetEvent('god_deliveries:startJob:client')

AddEventHandler('god:setJob', function(job)
	PlayerJob = job.name
end)

AddEventHandler('god_deliveries:setPlayerJob:client', function(job)
	print("Player job: " .. job)
	PlayerJob = job
end)

AddEventHandler('god_deliveries:startJob:client', function(deliveryType)
