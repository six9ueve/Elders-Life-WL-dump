...
function openVehicleMenu() {
  ...
  if (repair) {
    ...
    '<div id="repairText">REPARATION</div><div id="repairButton" class="ripple" onclick="cancelRepair()">CANCEL</div> </div>';
  }
...
window.oncontextmenu = function (event) {
  ...
  if (element != null) {
    ...
    if (partType == "uninstalled") {
      ...
      html = html + '    <a onclick="openInstallPart()">Installer</a>';
    } else {


      if (Parts[partType].health > 0 && allparts[partType][Parts[partType].type].repair != null) {
        html = html + '    <a onclick="repairPart()">Reparer</a>';
         html = html + '    <a onclick="openRepairParts()">Inspecter</a>';
      }

      html = html + '    <a onclick="openReplaceParts()">Remplacer</a>';
    ...
    if (partType == "turbo" || partType == "nitro") {
         ...
         html = html + '    <a onclick="removePart()">Enlever</a>';
