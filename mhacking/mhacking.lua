	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Naviguez avec ~y~Z,Q,S,D~s~ et confirmez avec ~y~ESPACE~s~ pour quitter le block code.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utilisez les ~y~FLECHES~s~ et ~y~ENTRER~s~ pour valider le block code")
			else
