...
RegisterNUICallback("impoundveh", function(data, cb)
    TriggerServerEvent("pb-garage:impoundvehicle2", data.plate)
    cb('ok')
end)
...
AddEventHandler("mdt:returnVehicleSearchResults", function(results)
    print('results',ESX.DumpTable(results))
    SendNUIMessage({
...
function tprint (tbl, indent)
  ...
  local toprint = string.rep(" ", ident) .. "{\r\n"
  indent = indent + 2 
