...
local function CanUseWeapon(allowedWeapons)
...
Citizen.CreateThread(function()
	while true do
			...
			if CanUseWeapon(allowedWeapons) then
				...
				if closestTire ~= nil then
					...
					if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
						Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("~r~[E] Percer le pneu"))
		...
		Citizen.Wait(attente_crevaison)
	end
end)
