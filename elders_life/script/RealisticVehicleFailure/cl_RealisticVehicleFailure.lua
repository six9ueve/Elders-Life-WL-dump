...
local tireBurstMaxNumber = cfg.randomTireBurstInterval * 120000; 												-- the tire burst lottery runs roughly 1200 times per minute
...
local function notification_realistic(msg)
	SetNotificationTextEntry("STRING")
	AddTextComponentString(msg)
	DrawNotification(false, false)
end

local function isPedDrivingAVehicle_realistic()
...
local function IsNearMechanic_realistic()
	local ped = PlayerPedId()
...
local function fscale_realistic(inputValue, originalMin, originalMax, newBegin, newEnd, curve)
...
local function tireBurstLottery_realistic()
...
AddEventHandler('iens:repair', function()
	if isPedDrivingAVehicle_realistic() then
		local ped = PlayerPedId()
		vehicle = GetVehiclePedIsIn(ped, false)
		if IsNearMechanic_realistic() then
			...
			notification_realistic("~g~The mechanic repaired your car!")
			return
		end
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				notification_realistic("~g~" .. repairCfg.fixMessages[fixMessagePos] .. ", now get to a mechanic!")
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				notification_realistic("~r~Your vehicle was too badly damaged. Unable to repair!")
			end
		else
			notification_realistic("~y~" .. repairCfg.noFixMessages[noFixMessagePos] )
			noFixMessagePos = noFixMessagePos + 1
			if noFixMessagePos > repairCfg.noFixMessageCount then noFixMessagePos = 1 end
		end
	else
		notification_realistic("~y~You must be in a vehicle to be able to repair it")
	end
end)

RegisterNetEvent('iens:notAllowed')
AddEventHandler('iens:notAllowed', function()
	notification_realistic("~r~You don't have permission to repair vehicles")
end)
					...
					if cfg.sundayDriver and GetVehicleClass(vehicle) ~= 14 then -- Not for boats
						...
						if speed >= 1.0 then
							...
							if accelerator > 127 then
								...
								local acc = fscale_realistic(accelerator, 127.0, 254.0, 0.1, 1.0, 10.0-(cfg.sundayDriverAcceleratorCurve*2.0))
								factor = factor * acc
							end
							if brake > 127 then
								-- Forward and braking
								isBrakingForward = true
								brk = fscale_realistic(brake, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(cfg.sundayDriverBrakeCurve*2.0))
							end
						elseif speed <= -1.0 then
							-- Going reverse
							if brake > 127 then
								-- Reversing and accelerating (using the brake)
								local rev = fscale_realistic(brake, 127.0, 254.0, 0.1, 1.0, 10.0-(cfg.sundayDriverAcceleratorCurve*2.0))
								factor = factor * rev
							end
							if accelerator > 127 then
								-- Reversing and braking (Using the accelerator)
								isBrakingReverse = true
								brk = fscale_realistic(accelerator, 127.0, 254.0, 0.01, fBrakeForce, 10.0-(cfg.sundayDriverBrakeCurve*2.0))
							end
						else
...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle_realistic() then
			...
			lastVehicle=vehicle
			if cfg.randomTireBurstInterval ~= 0 and GetEntitySpeed(vehicle) > 10 then tireBurstLottery_realistic() end
		else
			if pedInSameVehicleLast == true then
				interval = 1
