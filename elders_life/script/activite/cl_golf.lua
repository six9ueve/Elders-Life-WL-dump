	...
	while true do
		...
		DrawMarker(27,-1332.7823486328,128.18229675293,56.032329559326, 0, 0, 0, 0, 0, 0, 1.5, 1.5, 10.3, 0, 519, 0, 105, 0, 0, 2, 0, 0, 0, 0)
			...
			if distance < 1.5 then
				...
				if golfplaying then
					DisplayHelpText("Pressez ~g~~INPUT_CONTEXT~~s~ pour arreter le golf.")
				else
					DisplayHelpText("Pressez ~g~~INPUT_CONTEXT~~s~ pour jouer au golf")
				end
				if (IsControlJustReleased(1, 38)) then
					...
					if (golfplaying) then
						...
						TriggerEvent("customNotification","Pressez E pour swing, Q-D pour tourner, S pour changer de club.")
					end
				end
	...
	while golfstate == 1 do
		...
		if GetVehiclePedIsIn(GetPlayerPed(-1), false) == 0 then
			...
			if distance < 50.0 then
				DisplayHelpText("Allez à la balle de golf, pressez ~g~E~s~ pour faire tomber la balle si vous êtes coincé.")
				if ( IsControlJustReleased(1, 38) ) then
					dropShot()
				end
...
function endShot()
	...
	if distance < 1.5 then
		TriggerEvent("customNotification","Tu as la balle à portée!")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		TriggerEvent("customNotification","Tu as fait trop de tir..")
		totalgolfstrokes = golfstrokes + totalgolfstrokes
		golfstrokes = 0
		ballstate = 1
	end

	Citizen.Trace("La balle semblait avoir atterri sur: " .. GetCollisionNormalOfLastHitForEntity(mygolfball) )
end
...
function dropShot()
	...
	while doingdrop do
		...
		if distance < 50.0 and distanceHole > 50.0 then
			DisplayHelpText("Pressez ~g~E~s~ pour poser ici.")
			if ( IsControlJustReleased(1, 38) ) then
				doingdrop = false
			...
			DisplayHelpText("Pressez ~g~E~s~ pour poser - ~r~ trop loin de la balle ou trop près du trou.")
		end
	end
end
