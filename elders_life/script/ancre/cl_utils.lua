...
function onTooFast(vehicle)
    TriggerEvent('chat:addMessage', { args = {"^4[Ancre]", "^*^1Votre bateau va trop vite! Ralentissez pour jeter l'ancre!"} })
end

function onUnanchored(vehicle)
    TriggerEvent('chat:addMessage', { args = {"^4[Ancre]", "^*^0Vous avez levé l'ancre!"} })
end

function onAnchoring(vehicle)
    TriggerEvent('chat:addMessage', { args = {"^4[Ancre]", "^*^0Vous avez jeté l'ancre!"} })
    return true
end

function onAnchorBroke(vehicle)
    TriggerEvent('chat:addMessage', { args = {"^4[Ancre]", "^*^1L'ancre à cassé!"} })
end
