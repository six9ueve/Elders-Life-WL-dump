...
local carOnShow = GetHashKey('nero2')

local function IsTable(T)
	return type(T) == 'table'
end

local function SetIplPropState(interiorId, props, state, refresh)
...
Citizen.CreateThread(function ()
    while true do
        ...
        for _,location in ipairs(positions) do
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 6) then 
				casino_wait = 0
				DrawMarker(29, loc1.x, loc1.y, loc1.z+0.0, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, Red, Green, Blue, 200, 0, 0, 0, 1)

				DrawText3Ds(loc1.x, loc1.y, loc1.z + 0.3, '[~g~E~s~] Entrer dans le casino')
			...
			elseif CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc2.x, loc2.y, loc2.z, 6) then
				casino_wait = 0
				DrawMarker(29, loc2.x, loc2.y, loc2.z+0.5, 0, 0, 0, 0, 0, 0, 0.8, 0.8, 0.8, Red, Green, Blue, 200, 0, 0, 0, 1)
				DrawText3Ds(loc2.x, loc2.y, loc2.z + 0.8, '[~g~E~s~] Sortir du casino')
                if IsControlJustReleased(1, key_to_teleport) then
					DoScreenFadeOut(500)
    				Wait(500)
...
function spinMeRightRoundBaby()
	Citizen.CreateThread(function()
		...
		if not spinningObject or spinningObject == 0 or not DoesEntityExist(spinningObject) then
		  spinningObject = GetClosestObjectOfType(1100.0, 220.0, -51.0, 90.58, -1561087446, 0, 0, 0)
		  drawCarForWins()
		end
