...
RMenu.Add("gouv", "princ", RageUI.CreateMenu("Gouvernement", "~b~Menu job :", nil, nil, "aLib", "black"))
RMenu:Get("gouv", "princ").Closed = function()
    isMenuOpened = false
end

RMenu.Add("gouv", "vestiaire", RageUI.CreateMenu("Gouvernement", "~b~Menu job :", nil, nil, "aLib", "black"))
RMenu:Get("gouv", "vestiaire").Closed = function()
    isMenuOpened = false
end

RMenu.Add("gouv", "mixo", RageUI.CreateSubMenu(RMenu:Get("gouv", "princ"), "Gouvernement", "~b~Menu job :"))
RMenu:Get("gouv", "mixo").Closed = function()end

RMenu.Add("gouv", "palpe", RageUI.CreateSubMenu(RMenu:Get("gouv", "princ"), "Gouvernement", "~b~Menu job :"))
RMenu:Get("gouv", "palpe").Closed = function()end

RMenu.Add("gouv", "changetenue", RageUI.CreateSubMenu(RMenu:Get("gouv", "vestiaire"), "Gouvernement", "~b~Menu job :"))
RMenu:Get("gouv", "changetenue").Closed = function()end

RMenu.Add("gouv", "deletetenue", RageUI.CreateSubMenu(RMenu:Get("gouv", "vestiaire"), "Gouvernement", "~b~Menu job :"))
RMenu:Get("gouv", "deletetenue").Closed = function()end

local function openMenugouvF6()

    if isMenuOpened then return end
    isMenuOpened = true

    RageUI.Visible(RMenu:Get("gouv", "princ"), true)

    Citizen.CreateThread(function()
        while isMenuOpened do
            RageUI.IsVisible(RMenu:Get("gouv", "princ"),true,true,true,function() 
                local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                    gouvinteract = true else gouvinteract = false
                end 
                ...
                RageUI.ButtonWithStyle("Annonce Personnalisé", nil, {RightLabel = "→→→"}, not cooldown,function(a,h,s)
                    if s then
                        ...
                        if msg and msg ~= "" then
                            TriggerServerEvent('perso:Gouv', msg)                            
                        end
                    end
                end)               
                RageUI.Separator('~y~↓ Citoyen ↓')                
                RageUI.ButtonWithStyle("Facture", nil, {RightLabel = "→→→"}, gouvinteract,function(a,h,s)
                    if h then
                        MarquerJoueur()
                        if s then
                            ...
                            if amount ~= nil then
                                ...
                                TriggerServerEvent('god_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_juge', 'gouv', amount)
                            else
            ...
            RageUI.IsVisible(RMenu:Get("gouv", "palpe"),true,true,true,function()
...
local function openMenugouvVestiaire()

    if isMenuOpened then return end
    isMenuOpened = true

    RageUI.Visible(RMenu:Get("gouv", "vestiaire"), true)

    Citizen.CreateThread(function()
        while isMenuOpened do
            RageUI.IsVisible(RMenu:Get("gouv", "vestiaire"),true,true,true,function()
                RageUI.Separator('↓ Garde robe ↓')
                RageUI.ButtonWithStyle("Tenue de Travail", nil, {RightLabel = "→→→"}, true,function(a,h,s)
                    if s then   
                            TriggerEvent('skinchanger:getSkin', function(skin)  
                                if skin.sex == 0 then
                                    TriggerEvent('skinchanger:loadClothes', skin, ConfigGouv.uniform.male)
                                else
                                    TriggerEvent('skinchanger:loadClothes', skin, ConfigGouv.skin_female)
                                end
                            end)
                    end
                end) 
                ...
                end, RMenu:Get("gouv", "changetenue"))
                ...
                end, RMenu:Get("gouv", "deletetenue"))
                RageUI.Separator('↓ Tenues ↓')
            end, function()end, 1)
            RageUI.IsVisible(RMenu:Get("gouv", "changetenue"),true,true,true,function()
            ...
            RageUI.IsVisible(RMenu:Get("gouv", "deletetenue"),true,true,true,function()
...
Citizen.CreateThread(function()
    while true do
        interval = 750
        for k, v in pairs(ConfigGouv.points) do
...
RegisterKeyMapping('gouv', 'Menu Job : gouv', 'keyboard', 'F6')