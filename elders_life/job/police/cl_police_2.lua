...
RegisterNetEvent("EldersPolice:radarRemoveBlips")
AddEventHandler("EldersPolice:radarRemoveBlips", function(RemoveBlips, BlipsID, BlipsCoords)
...
RegisterNetEvent("EldersPolice:TakeAppelHelp")
AddEventHandler("EldersPolice:TakeAppelHelp", function(pos, job)
    ...
    AddTextComponentString("Réclamation renfort | LSPD")
    EndTextCommandSetBlipName(BlipsRenfort)
    Wait(70 * 1000)
    RemoveBlip(BlipsRenfort)
end)

RegisterNetEvent("EldersPolice:SendSoundPlaque")
AddEventHandler("EldersPolice:SendSoundPlaque", function()
...
RegisterNetEvent("EldersPolice:LSPDAlerte")
AddEventHandler("EldersPolice:LSPDAlerte", function(coords, raison, matricule)
...
function spawnHelicoLSPD(ped, car)
...
function ChangeCameraLSPD(x,y,z,r)
    camera = true
    DoScreenFadeOut(800)
    cam = CreateCam("DEFAULT_SCRIPTED_CAMERA", 1)
...
local i 						= 0
local timeout					= 0
local RadarInfosLSPD = {}
local Flashled = {}
local RadarPlaced = {}

function RadarLSPD(m)
    if isRadarPlaced then 
        ...
        if maxSpeed ~= nil and maxSpeed > 90 then
            ...
            isRadarPlaced = true
            TriggerServerEvent("EldersPolice:AddInfosRadar", RadarPos,maxSpeed)
            ...
            TriggerServerEvent("EldersPolice:AddRadarPlaced", RadarPlaced)
...
Citizen.CreateThread(function()
        while true do
            ...
            if IsPedInAnyVehicle(PlayerPedId(), false) and GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId(), false), -1) == PlayerPedId() then
                ESX.TriggerServerCallback("EldersPolice:GetInfosRadar", function(radarinfos)
                    RadarInfosLSPD = radarinfos
                end)
                for k,v in pairs(RadarInfosLSPD) do
                                ...
                                if PedSpeed > speedlimit then
                                    ...
                                    if is_flashed == false then
                                        ...
                                        if ESX.PlayerData.job.name == "police" or ESX.PlayerData.job.name == "sheriff" then
                                            ...
                                            TriggerServerEvent("EldersPolice:AddFlashSpeed", Flashled)
                                            ...
                                            TriggerServerEvent("EldersPolice:SendBillToOnwnerVeh", Flashled.plaque, Flashled.price)
                                            TriggerServerEvent("EldersPolice:SendNotifFlashedLSPD")
                                            --TriggerServerEvent("EldersPolice:SendNotifFlashedBCSO")
                                            is_flashed = true
                                            timeout = 85
...
AddEventHandler('elders_police:toggleSpeedzoneDraw', function(boolean)
...
local sizeMax = 50.0
AddEventHandler('elders_police:promptSpeedzone', function()
    ...
    repeat
        ...
        if IsControlJustReleased(1, 194) or IsPedInAnyVehicle(PlayerPedId(), true) then -- Cancel
            ...
            InDrawSpeedzones()
            TriggerEvent('elders_police:finishedPlacingSpeedzone')
            return
        end

        Citizen.Wait(0)
    until placed
    TriggerServerEvent('elders_police:addSpeedzone', GetEntityCoords(PlayerPedId()), size)
    DrawSpeedzones = BeforougerawSpeedzones
    InDrawSpeedzones()
end)

RegisterNetEvent('elders_police:addSpeedzone')
AddEventHandler('elders_police:addSpeedzone', function(whoCreated, pos, size)
...
RegisterNetEvent('elders_police:removeSpeedzone')
AddEventHandler('elders_police:removeSpeedzone', function(whoDeleted, deletedSpeedZone)
        ...
        while DrawSpeedzones do
            ...
            if closest and closestDistance < closest.size then
                ...
                if IsControlJustReleased(1, 178) then
                    TriggerServerEvent('elders_police:removeSpeedzone', closest)
                end
            end
        end
    end)
