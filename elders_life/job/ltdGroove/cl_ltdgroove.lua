...
RMenu.Add("LTD", "princ", RageUI.CreateMenu("LTD", "~b~Menu job :", nil, nil, "aLib", "black"))
RMenu:Get("LTD", "princ").Closed = function()
    isMenuOpened = false
end

RMenu.Add("LTD", "vestiaire", RageUI.CreateMenu("LTD", "~b~Menu job :", nil, nil, "aLib", "black"))
RMenu:Get("LTD", "vestiaire").Closed = function()
    isMenuOpened = false
end

RMenu.Add("LTD", "changetenue", RageUI.CreateSubMenu(RMenu:Get("LTD", "vestiaire"), "LTD", "~b~Menu job :"))
RMenu:Get("LTD", "changetenue").Closed = function()end

RMenu.Add("LTD", "deletetenue", RageUI.CreateSubMenu(RMenu:Get("LTD", "vestiaire"), "LTD", "~b~Menu job :"))
RMenu:Get("LTD", "deletetenue").Closed = function()end

local function openMenuLtdGroovF6()

    if isMenuOpened then return end
    isMenuOpened = true

    RageUI.Visible(RMenu:Get("LTD", "princ"), true)

    Citizen.CreateThread(function()
        while isMenuOpened do
            RageUI.IsVisible(RMenu:Get("LTD", "princ"),true,true,true,function()  
                local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                if closestPlayer ~= -1 and closestDistance <= 3.0 then
                    LtdGroovinteract = true else LtdGroovinteract = false
                end 
                    ...
                    if h then
                        ...
                        if s then
                            ...
                            if amount ~= nil then
                                ...
                                TriggerServerEvent('god_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ltdgroove', 'LTD Groove', amount)
                            else
...
local function openMenuLtdGroovVestiaire()

    if isMenuOpened then return end
    isMenuOpened = true

    RageUI.Visible(RMenu:Get("LTD", "vestiaire"), true)

    Citizen.CreateThread(function()
        while isMenuOpened do
            RageUI.IsVisible(RMenu:Get("LTD", "vestiaire"),true,true,true,function()
                ...
                end, RMenu:Get("LTD", "deletetenue"))
                RageUI.Separator('↓ Tenues ↓')
            end, function()end, 1)
            RageUI.IsVisible(RMenu:Get("LTD", "changetenue"),true,true,true,function()
            ...
            RageUI.IsVisible(RMenu:Get("LTD", "deletetenue"),true,true,true,function()
...
Citizen.CreateThread(function()
    while true do
        interval = 750
        for k, v in pairs(ConfigLTDgroove.points) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == "ltdgroove" then
...
RegisterCommand('LTD', function()
    if ESX.PlayerData.job.name == "ltdgroove" then
        openMenuLtdGroovF6()
    end
end)

RegisterKeyMapping('LTD', 'Menu Job : LTD', 'keyboard', 'F6')