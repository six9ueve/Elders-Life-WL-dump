-- Optimizations
local showMap = true, showBars, showArmor, showOxygen, isOpen, cinematicHud, isPaused
...
Citizen.CreateThread(function()
	while true do
	Citizen.Wait(2500)
		TriggerEvent('god_status:getStatus', 'hunger', function(h)
			hunger = h.getPercent()
		end)
		TriggerEvent('god_status:getStatus', 'thirst', function(e)
			thirst = e.getPercent()
		end)
		if Config.useStress then
			TriggerEvent('god_status:getStatus', 'stress', function(g)
				stress = g.getPercent()
			end)
			TriggerEvent('god_status:getStatus', 'drunk', function(f)
				stamina = f.getPercent()
			end)
		end
	end
end)
...
RegisterKeyMapping('hud', 'Ouvrir Menu HUD', 'keyboard', Config.openKey)
...
RegisterNetEvent('god:playerLoadedHUD')
AddEventHandler('god:playerLoadedHUD', function(xPlayer)
	SendNUIMessage({action = 'hungerShow'})
	SendNUIMessage({action = 'thirstShow'})
	SendNUIMessage({action = 'stressShow'})
	SendNUIMessage({action = 'healthShow'})
