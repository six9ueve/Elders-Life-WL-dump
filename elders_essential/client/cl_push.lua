...
Config = {} 
Config.FuelNeeded = 4.0 -- 100.0 being broken and 1000.0 being fixed a lower value than 100.0 will break it
Config.MotorNeeded = 0.0 -- 100.0 being broken and 1000.0 being fixed a lower value than 100.0 will break it
...
Citizen.CreateThread(function()
    while true do 
        ...
        if Vehicle.Vehicle ~= nil then
                ...
                if IsVehicleSeatFree(Vehicle.Vehicle, -1) and (GetVehicleFuelLevel(Vehicle.Vehicle) <= Config.FuelNeeded or GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.MotorNeeded ) then
                    ESX.Game.Utils.DrawText3D({x = Vehicle.Coords.x, y = Vehicle.Coords.y, z = Vehicle.Coords.z}, 'Pressez [~g~SHIFT~w~] et [~g~E~w~] pour pousser le véhicule', 0.4)
                end

            if IsControlPressed(0, Keys["LEFTSHIFT"]) and IsVehicleSeatFree(Vehicle.Vehicle, -1) and not IsEntityAttachedToEntity(ped, Vehicle.Vehicle) and IsControlJustPressed(0, Keys["E"])  and (GetVehicleFuelLevel(Vehicle.Vehicle) <= Config.FuelNeeded or GetVehicleEngineHealth(Vehicle.Vehicle) <= Config.MotorNeeded ) then
